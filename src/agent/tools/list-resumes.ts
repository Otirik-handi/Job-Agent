...
export const listResumesTool = createDomainTool({
  name: 'listResumes',
  description: '列出系统中已导入的全部简历，供获取 resumeId 复用。用户要求分析简历、匹配岗位或生成专属简历但未提供 resumeId 时，先调用本工具发现系统简历。无参数，只读无副作用。返回 count 与简历列表（resumeId、name、sourceType、analyzed 是否已分析）。',
  inputSchema,
