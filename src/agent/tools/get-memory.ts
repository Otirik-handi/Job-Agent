...
export const getMemoryTool = createDomainTool({
  name: 'getMemory',
  description: '读取 Agent 持久记忆：传 label 读取单块，不传则读取全部记忆块。参数 label 可选，取值 resume（简历画像）/ preferences（用户偏好）/ status_scratchpad（投递进度速记），缺省返回全部。需要回忆历史事实（用户偏好、简历画像、投递进度）时先调用本工具，记忆内容一律以返回为准，不臆测不编造；只读无副作用。返回 count 与各记忆块（label、value、description、字符上限 limit）。',
  inputSchema,
