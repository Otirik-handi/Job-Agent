...
export const applyJobInputSchema = z.strictObject({
  jobOpportunityId: z.string().min(1).describe('岗位 ID'),
  action: z.enum(['apply', 'skip']).describe('apply：投递推进（matched→applying→applied）；skip：跳过（仅 applied 已投递状态不可跳过，其余状态含终态均可跳至 skipped）'),
