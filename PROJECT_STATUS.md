...
## 已完成：外部 Skill 炼化（2026-08-13 启动，12/12 全部完成）

> 背景：通过 find-skills 检索求职领域 agent skill 生态，筛选后安装至 `external-skills/`（4 仓库，嵌套 git 独立更新，`.gitignore` 排除不入库，提交 `5dfdf16`），逐个精读并产出「炼化结论文档」供后续落地参考。**本次炼化任务仅产出文档，落地工作待炼化完成后按文档统一规划（用户明确）。**
...
| 12 | offer-comparison-analyzer | 总薪酬计算器（Year 1 vs Ongoing 双口径 + perks 货币化）+ 非金钱四维（成长子项分解评分/工作生活/团队文化含经理质量/风险）+ 加权矩阵 + 红旗清单三组 18 条（Offer/Company/Role）+ 决策心理测试 4 组 | **覆盖率最高一篇**：offer-evaluation 已对齐通用形态，增量在细节级（红旗 18 条/双口径/心理测试为核心）；方案一增量增强为推荐；#8 预留的 base vs 总包权衡由本篇承接确认 |
...
- **期望薪资 deflection 三处同源印证**（#8/#9/#10）：面试被问（#9 难题库）、谈薪被问（#8 场景 2）、表单期望薪资字段（#10 Type 4 延伸）——三处话术合并立项，建议归 negotiation
- **主动触达能力空白**（#11）：channel-discovery 只发现渠道不生成触达内容；国内三形态（BOSS 打招呼/私信/邮件）
- **表单填写能力空白**（#10）：7 类题型框架 + 长度校准表，方案一新增内置 skill
- **故事银行系统化**（#9）：5 类能力清单 + 三版本粒度 + 难题公式（weakness/失败）；prepare-interview 契约可加 probability/redFlags 字段
- **投递-版本关联**（#6 §6）：痛点"我投 X 用的哪个版本"当前答不了——actions 审计携带 tailoredResumeId（方案 α 零迁移）或表加字段
- **offer 评估细节级增量**（#12）：红旗清单 18 条 / Year 1 vs Ongoing 双口径 / 心理测试 4 组 / 成长子项评分
- **无岗位独立打磨场景**（#4/#5）：analyze-resume 只给建议、tailoredResume 须有岗位——无岗位时无法产出改写文本
- **结构级重排能力**（#2 §6）：edits 仅文本替换，经历区块重排/技能换序需 editType 扩展
- **简历数据模型升级联动**：master 素材库（#6 §5.1）、结构级重排（#2 §6）、投递-版本关联、故事库数据化（#9 方案二）同属简历模型议题，任一项立项时一起评估
- **中文词表本地化**（#1/#3/#5/#8-#12）：red flag 短语、行业词表、强动词库、谈判/触达/表单话术原文为英文，落地均需中文本地化
- **无岗位独立打磨场景**（#4/#5）：analyze-resume 只给建议、tailoredResume 须有岗位——无岗位时无法产出改写文本
- **结构级重排能力**（#2 §6）：edits 仅文本替换，经历区块重排/技能换序需 editType 扩展
