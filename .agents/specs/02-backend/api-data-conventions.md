...
## 会话摘要（conversations.summary）

- `conversations.summary`（text，可空）存会话级滚动摘要：会话消息数首次达到轮数上限时，对将被截断的旧轮由 LLM 生成一次压缩摘要，此后常驻，随上下文注入
- 内容侧重（compaction 原则）：用户偏好与画像变化、已投递进度、未决事项/进行中任务、关键决策；丢弃冗余工具输出
- 生成时机：会话消息数首次达到轮数上限（`MAX_HISTORY_ROUNDS=12` 截断点）时，对将被截断的旧轮生成摘要；只生成一次——`summary` 非空后不再重复生成，不滚动更新
- 生成失败降级：摘要生成失败不阻塞请求（跳过本次摘要，后续轮次可再尝试或放弃）
- 注入位置：摘要作为上下文段注入，位于稳定段（SYSTEM_PROMPT/记忆/Skill）之后、最近 N 轮之前
- 与记忆层边界：`summary` = 对话上下文的压缩（过程性内容、未决事项）；`memory_blocks` = 结构化事实（偏好/画像/进度）——两者不混用；原始消息全量落库可溯源，摘要失真可回查
- 敏感信息：摘要生成入参为消息文本（用户数据），LLM 调用不落日志；摘要内容只注入本会话上下文，不外发
- 为什么：轮数截断后旧轮语义丢失，一次性摘要以低成本保住跨轮上下文；只生成一次避免重复开销与内容漂移；失败降级保证摘要不阻塞主流程（对齐设计 P1-4 compaction 定稿）
